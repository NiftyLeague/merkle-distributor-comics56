...
const toNode = (index: number | BigNumber, account: string, amount0: BigNumber, amount1: BigNumber): Buffer => {
  const pairHex = utils.solidityKeccak256(['uint256', 'address', 'uint256', 'uint256'], [index, account, amount0, amount1])
...
const verifyProof = (
  ...
  let pair = toNode(index, account, amount0, amount1)
...
const getRoot = (balances: { account: string; amount0: BigNumber; amount1: BigNumber; index: number }[]): Buffer => {
  let nodes = balances
    .map(({ account, amount0, amount1, index }) => toNode(index, account, amount0, amount1))
...
let balances: { index: number; account: string; amount0: BigNumber; amount1: BigNumber }[] = []
let valid = true
...
Object.keys(json.claims).forEach((address) => {
  ...
  balances.push({ index: claim.index, account: address, amount0: BigNumber.from(claim.amount0), amount1: BigNumber.from(claim.amount1) })
  if (verifyProof(claim.index, address, claim.amount0, claim.amount1, proof, merkleRoot)) {
